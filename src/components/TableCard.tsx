...
export default function TableCard({ table, date, duration, selected, onSelect }: { table: Table; date: string; duration: number; selected: boolean; onSelect: () => void }) {
  ...
  }, [table.id, date, duration, isOpen]);
  const has = !isOpen && (avail ?? 0) > 0;
  return (
    <button onClick={onSelect} disabled={avail !== null && !has}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          {avail === null ? <div className="w-16 h-3 rounded animate-shimmer mt-1" /> : <p className={`text-xs mt-0.5 ${has ? "text-emerald-400" : "text-red-400"}`}>{isOpen ? "Только на месте" : has ? `${avail} слотов` : "Нет мест"}</p>}
        </div>
      </div>
