...
interface CB { booking_id: string; table_name: string; hall: string; date: string; time: string; duration: number; price: number; customer_name: string; phone: string; }
...
export default function SuccessPage() {
  ...
  const addCal = () => {
    if (!bk) return;
    const s = new Date(`${bk.date}T${bk.time}:00`); const e = new Date(s.getTime() + bk.duration * 3600000);
  ...
  return (
    <div className="mx-auto max-w-[430px] px-4 flex flex-col items-center justify-center min-h-dvh py-8 animate-fade-in">
      ...
      <div className="w-full mb-6"><BookingSummaryCard items={[{ label: "Стол", value: bk.table_name }, { label: "Зал", value: bk.hall }, { label: "Дата", value: fmtDate(bk.date) }, { label: "Время", value: `${bk.time} — ${endTime(bk.time, bk.duration)}` }, { label: "Клиент", value: bk.customer_name }, { label: "Телефон", value: bk.phone }, { label: "Оплачено", value: `${bk.price.toLocaleString()} ₸`, highlight: true }]} /></div>
